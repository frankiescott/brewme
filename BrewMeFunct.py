...
#Generate distance matrix data from api queries
	urls = [requests.get("https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=" + "+".join(addr.split()) + "," + "+".join(city.split()) + "," + "+".join(state.split('_')) + "&destinations=" + i + "&key=AIzaSyBmz6G1sB05RhTPk60jNoY3NbiNSPxmCkw").json() for i in loc_data]
	#print(len(urls))
...
#filter brewery data down to breweries within our selected distance
	...
	for i in range(len(loc_data)+1):
		...
		for j in range(i+1,len(loc_data)+1):
			val = requests.get("https://maps.googleapis.com/maps/api/distancematrix/json?units=imperial&origins=" + map_data[i] + "&destinations=" + map_data[j] + "&key=AIzaSyBmz6G1sB05RhTPk60jNoY3NbiNSPxmCkw").json()
	...
	place = [requests.get("https://maps.googleapis.com/maps/api/place/findplacefromtext/json?input="+'%20'.join(i["name"].split())+"&inputtype=textquery&fields=place_id,photos,formatted_address,name,rating,opening_hours,geometry&key=AIzaSyBmz6G1sB05RhTPk60jNoY3NbiNSPxmCkw").json() for i in JS]

	#obtain detailed data based on the place id generated by place array
	details = [requests.get("https://maps.googleapis.com/maps/api/place/details/json?place_id="+i["candidates"][0]["place_id"]+"&fields=website,formatted_phone_number,rating&key=AIzaSyBmz6G1sB05RhTPk60jNoY3NbiNSPxmCkw").json() for i in place]
